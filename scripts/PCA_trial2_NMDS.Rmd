---
title: 'PCA (trial 2) and NMDS: Methods Paper '
...
# PCA 1: most of the continuous, numberical variables
...
## PCA 1 - Visualization and Table of Eigenvalues
```{r}
...
## PCA 2 - Visualization and Table of Eigenvalues
```{r}
...
## PCA2 - Side-by-side Comparison of PCA for Flam. Metrics for EPI and HP
...
ggsave('figures/PCAflammetrics.jpg', height = 8, width = 12)
```
...
ggsave('figures/pca.sidebysideV2.jpg', plot = PCA.epi.and.hp, height = 7.5, width = 16.5)
```
...
ggsave('figures/PCAflammetrics.scaled.centered.jpg', height = 8, width = 12)
```
...
pca.table.df %>%
  ...
  save_kable('figures/pca_table_BOTH.html')
```
...
First, making dataframe with eigenvalues for principal components, proportion of variance explained and cumulative proportion:
```{r}
eig1.epi <- c(-0.44, 0.03, -0.90, 0.00, -0.08, -0.97, -0.51, 0.01, 33.19, 33.19)
eig2.epi <- c(-0.62, -0.68, -0.08, 0.59, -0.72, 0.06, 0.70, -0.06, 25.09, 58.28)
eig3.epi <- c(0.14, -0.26, -0.10, 0.00, 0.15, 0.00, 0.11, -0.96, 16.04, 74.31)
###
eig1.hp <- c(0.02, 0.57, 0.09, -0.41, -0.64, -0.34, -0.83, 0.00, 30.72, 30.72)
eig2.hp <- c(-0.76, -0.15, 0.07, 0.81, -0.06, -0.02, 0.21, 0.62, 21.54, 52.26)
eig3.hp <- c(0.05, -0.31, -0.94, -0.08, -0.12, -0.89, -0.24, 0.00, 17.94, 70.19)
...
pca.table.df2 %>%
  ...
  save_kable('figures/pca_table_SIDEbySIDE.html')
```
