...
```{r}
seki_all_methods <- read_csv(here("processed-data", "compiled-datasets", "SEKI", "seki_flam_data_all.csv"), show_col_types = FALSE) %>% 
  clean_names() %>% 
  mutate(lfm = lfm_n_as_imputed) %>% 
  select(2:49, flam_index, max_mpa_sample, -ttms, -bottle_wt, lfm) %>%  #drop columns we don't want
...
#seki_all_methods[is.na(seki_all_methods$ttms), ]  
```

# LOCAL DATA: 

```{r}
local_all_methods <- read_csv(here("processed-data", "compiled-datasets", "LOCAL", "local_flam_data_all.csv"), show_col_types = FALSE) %>% clean_names() %>% 
  mutate(lfm = lfm_n_as_imputed) %>% 
  select(5:8, 12:44, 53:61, flam_index, max_mpa_sample, lfm) %>%  #drop columns we dont want
  select(-year_month_spp, -model_spp_bins10lfm_gr_nodate, -individual_bins10lfm_gr_nodate, -prop_ignite_bins10_nodate) %>% 
...
# Writing csv. files
```{r, include = FALSE}
write_csv(paired_methods_long2, here("processed-data", "compiled-datasets", "METHODS", "bothmethods_bothlocations_long.csv"))

write_csv(paired_methods_long, here("processed-data", "compiled-datasets", "METHODS", "bothm_bothl_manual_ignitions_in.csv"))

write_csv(paired_methods_wide2, here("processed-data", "compiled-datasets", "METHODS", "bothmethods_bothlocations_wide.csv"))
```
