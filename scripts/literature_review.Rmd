...
data <- read_csv(here('raw-data', 'methods.lit.review.csv'))[1:158,]
location.data <- read_csv(here('raw-data', 'methods.lit.review.locations.csv'))[1:184,]
```

Global ignitions datasets
NOTE: these datasets are very large. They are too large to push to the GitHub in their raw form, so if they are downloaded, they must be deleted from the remote prior to committing and pushing changes to GitHub. If they are left on the remote, an error will pop up saying that the file is too large. Also, (as if that first part wasn't already annoying enough) these urls will expire on October 29, 2022 and then I will need to request them from Global Fire Atlas again to get new urls. These complications unfortunately make this code difficult to distribute.
```{r}
# Ignitions (2016) Dataset from global fire atlas
temp <- tempfile()
url <- 'https://daac.ornl.gov/orders/a97981aeff140af7f473132ebfc67fde/CMS_Global_Fire_Atlas/data/Global_fire_atlas_V1_ignitions_2016.zip' # Note: this URL may expire after 30 days (in late October)
...
temp <- tempfile()
url <- 'https://daac.ornl.gov/orders/a97981aeff140af7f473132ebfc67fde/CMS_Global_Fire_Atlas/data/Global_fire_atlas_V1_ignitions_2015.zip' # Note: this URL may expire after 30 days (in late October)
download.file(url, temp)
...
ggsave('figures/lit.review.barplot.jpg', height = 7, width = 12.64)
```
...
ggsave('figures/lit.review.epi.weights.jpg', height = 5, width = 7.5)
```
...
ggsave('figures/lit.review.all.methods.weights.jpg', height = 5, width = 7.5)
```
...
ggsave('figures/lit.review.locations.all.ignitions.jpg', height = 7, width = 12.64)
```
...
ggsave('figures/lit.review.locations.FG.ignitions.jpg', height = 7, width = 12.64)
```
...
ggsave('figures/lit.review.locations.G.ignitions.jpg', height = 7, width = 12.64)
```
